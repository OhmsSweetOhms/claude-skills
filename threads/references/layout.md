...
<repo-root>/threads/
  ...
  reviews/review-<YYYY-MM-DD>.md  # status-review snapshot (Status review workflow output)
...
| `<thread>/handoff.md` | Session-to-session hand-off, in two parts with two cadences. The **`## Current truth` block** is the only forward-facing state — Focus / PROVED / OPEN / NEXT / Baseline (green) / RULED OUT — **overwritten** (never appended) at every plan-hop change and every wrap, in the same commit, and bounded at **8,192 B** by the pre-commit guard. The **Session log** below it is append-only, newest entry on top, never edited. A `## Reading order for a cold start` section sits between them. Created at thread birth; rewritten by the New-plan-hop and Close-thread workflows and by a wrap (SKILL.md invariant "Handoff tracks plan state"), and on user request for in-session notes. Distinct lane from README (structural) and findings (point-in-time snapshot). |
...
|--------|---------|
| `active` | In progress. |
| `blocked` | Awaiting external input (research, decision, hardware) — and the state of a plan that is **drafted but not fired**: its hop is `blocked`, with what it waits on in `notes`. A drafted plan always gets a hop; there is no `drafted` status. |
