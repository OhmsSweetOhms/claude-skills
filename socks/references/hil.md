...
For projects that need more than one firmware personality on the same
bitstream, keep `hil.json` as the default and place sibling configs such as
`hil-a53.json` or `hil-r5.json` beside it. Select one per run with:

```bash
python scripts/socks.py --project-dir <project> --stages 16,17 --hil-config hil-r5.json
```

The override is intentionally file-level: every existing `hil.json` remains
valid, and stages that do not specify `--hil-config` continue to load
`<project>/hil.json`. The orchestrator exports `SOCKS_HIL_CONFIG` for child
HIL scripts; direct script users may set that environment variable when they
...
Profile manifest markers are applied only to `firmware.flow: "no_os_make"`
configs by default. Native Vitis/R5 firmware variants on the same ADI
bitstream use their own `firmware.pass_marker` or `pass_markers` so a stale
no-OS profile state file cannot make Stage 17 wait for ADI JESD UART text from
