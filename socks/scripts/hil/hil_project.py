...
def main() -> int:
    ...
    if project_scope == "system" and build_cfg.get("flow", "vivado_native") == "adi_make":
        if not build_cfg.get("recipe"):
            print(f"\n  ERROR: build.flow=adi_make requires socks.json::build.recipe "
                  f"(the adi selection block is retired -- one-pointer ruling 2026-07-25; "
                  f"point build.recipe at platforms/profiles/<profile>/build-recipe.json)")
            return 1
        try:
