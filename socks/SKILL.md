...
**Stage 18 ARM Debug flow:** Stage 18 uses a persistent XSDB debug session
(`XSDBSession` in `hil_lib.py`) for breakpoint-paced ILA captures. Key points:
- Firmware must structure test phases as separate functions (one per capture)
- Each capture in `ila_trigger_plan.json` needs `break_before` (function name)
  and/or `break_before_addr` (hex address)
- `hil.json` must have a `debug` section (auto-generated by `hil_prep.py`)
- On capture failure: automatic backtrace, variable dump, AXI fault check
- On CPU fault: JTAG-to-AXI register dump fallback via `DUMP_AXI` command
- UART is a pure debug log channel (no G-byte pacing)
- Debug builds use `-g -O0 -fno-omit-frame-pointer` for reliable breakpoints
