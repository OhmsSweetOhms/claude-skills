...
# 0xFFFC0000). PS bring-up chain also HW-verified (2026-06-30): the AR 76051 halt,
# the DDR/IO PLL power-up (PLL_PWRDWN clear; PLL_STATUS 0x39->0x3F, DDRC_mode->1),
# ps7_init, the rst -processor MMU drop, and DRAM read/write. The PROVEN VEHICLE for
# the flow is the workbench dashboard (references/zynq-jtag-flash-workbench.md); this
# standalone script has NOT been run end-to-end as a whole — see
# references/jtag-flash-bootmode-independent.md.
# NOTE: an incompatible ps7_init.tcl (wrong board) can WEDGE the debug DAP/TAP and
# require a physical power-cycle — use the .xsa/ps7_init that matches the board.
#
